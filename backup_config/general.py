...
@job("queries", timeout=120, ttl=180)  # Increased timeout to 120 seconds
def test_connection(data_source_id):
    try:
        data_source = models.DataSource.get_by_id(data_source_id)
        logger.info("Testing connection for data source %s (%s)", data_source.name, data_source.type)
        data_source.query_runner.test_connection()
        logger.info("Connection test successful for data source %s", data_source.name)
    except Exception as e:
        logger.error("Connection test failed for data source %s: %s", data_source.name, str(e))
        return e
    else:
        return True
